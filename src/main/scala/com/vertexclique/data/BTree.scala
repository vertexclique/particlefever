...
abstract sealed class BTree[+A <% Ordered[A]] {
   ...
   * Space - O(log n)
   */
  def add[B >: A <% Ordered[B]](x: B): BTree[B] =
    if (isEmpty) BTree.make(x)
    else if (x < value) BTree.make(value, left.add(x), right)
    else if (x > value) BTree.make(value, left, right.add(x))
    else this
   ...
   * Space - O(log n)
   */
  def remove[B >: A <% Ordered[B]](x: B): BTree[B] =
    if (isEmpty) fail("Can't find " + x + " in this tree.")
    else if (x < value) BTree.make(value, left.remove(x), right)
    else if (x > value) BTree.make(value, left, right.remove(x))
    else {
      if (left.isEmpty && right.isEmpty) BTree.empty
      else if (left.isEmpty) right
      else if (right.isEmpty) left
      else {
        val succ = right.min
        BTree.make(succ, left, right.remove(succ))
      }
    }
  ...
  def contains[B >: A <% Ordered[B]](x: B): Boolean = {
    def loop(t: BTree[A], c: Option[A]): Boolean =
   ...
   * Space - O(log n)
   */
  def subtree[B >: A <% Ordered[B]](x: B): BTree[B] =
    if (isEmpty) fail("Can't find " + x + " in this tree.")
   ...
   * Space - O(log n)
   */
  def isSubtree[B >: A <% Ordered[B]](t: BTree[B]): Boolean = {
    def loop(a: BTree[B], b: BTree[B]): Boolean =
   ...
   * Space - O(log n)
   */
  def merge[B >: A <% Ordered[B]](t: BTree[B]): BTree[B] = {
    def loop(s: BTree[B], d: BTree[B]): BTree[B] =
      if (s.isEmpty) d
   ...
   * Space - O(log n)
   */
  def map[B <% Ordered[B]](f: (A) => B): BTree[B] =
    if (isEmpty) BTree.empty
    else BTree.make(f(value), left.map(f), right.map(f))
   ...
   * Space - O(log n)
   */
  def invert[B >: A](implicit num: Numeric[B]): BTree[B] =
    if (isEmpty) BTree.empty
    else BTree.make(num.negate(value), right.invert(num), left.invert(num))
  ...
  def successor[B >: A <% Ordered[B]](x: B): A = {
    def forward(t: BTree[A], p: List[BTree[A]]): A =
    ...
    def backward(t: BTree[A], p: List[BTree[A]]): A =
   ...
   * Space - O(log n)
   */
  def path[B >: A <% Ordered[B]](x: B): List[BTree[A]] =
    if (isEmpty) fail("Can't find " + x + " in this tree.")
  ...
  def takeLargest(n: Int): List[A] = {
    def loop(t: BTree[A], l: List[A]): List[A] =
   ...
   */
  def diameter: List[A] = {
    def build(t: BTree[A], p: List[A]): List[A] =
      if (t.isEmpty) p
  ...
  def valuesByDepth: List[A] = {
    def loop(s: List[BTree[A]]): List[A] =
      if (s.isEmpty) Nil
  ...
  def valuesByBreadth: List[A] = {
    ...
    def loop(q: Queue[BTree[A]]): List[A] =
      if (q.isEmpty) Nil
  ...
  def valuesByZigZag: List[A] = {
    def zig(ltr: List[BTree[A]], rtl: List[BTree[A]]): List[A] =
    ...
    def zag(ltr: List[BTree[A]], rtl: List[BTree[A]]): List[A] =
      if (ltr.isEmpty && rtl.isEmpty) Nil
  ...
  def rootToLeafPathsWithSum[B >: A](sum: B)(implicit num: Numeric[B]): List[List[B]] = {
    def loop(t: BTree[A], p: List[B]): List[List[B]] =
      if (t.isEmpty)
...
case object Leaf extends BTree[Nothing] {
  ...
  def left: BTree[Nothing] = fail("An empty tree.")
  def right: BTree[Nothing] = fail("An empty tree.")
  def size: Int = 0
...
object BTree {
   ...
   * Space - O(1)
   */
  def make[A <% Ordered[A]](x: A, l: BTree[A] = Leaf, r: BTree[A] = Leaf): BTree[A] =
   ...
   * Space - O(log n)
   */
  def apply[A <% Ordered[A]](xs: A*): BTree[A] = {
    var r: BTree[A] = BTree.empty
    for (x <- xs) r = r.add(x)
    r
  }
   ...
   * Space - O(log n)
   */
  def fromSortedArray[A <% Ordered[A]](a: Array[A]): BTree[A] = {
    def loop(l: Int, r: Int): BTree[A] =
      if (l == r) BTree.empty
      else {
        val p = (l + r) / 2
        BTree.make(a(p), loop(l, p), loop(p + 1, r))
      }
   ...
   * Space - O(log n)
   */
  def fromSortedList[A <% Ordered[A]](l: List[A]): BTree[A] = {
    def loop(ll: List[A], n: Int): (List[A], BTree[A]) =
      if (n == 0) (ll, BTree.empty)
      else {
        val (lt, left) = loop(ll, n / 2)
        val (rt, right) = loop(lt.tail, n - 1 - n / 2)
        (rt, BTree.make(lt.head, left, right))
      }
   ...
   * Space - O(log n)
   */
  def complete[A <% Ordered[A]](x: A, d: Int): BTree[A] =
    if (d == 0) BTree.make(x)
    else {
      val t = BTree.complete(x, d - 1)
      BTree.make(x, t, t)
    }
   ...
   * Space - O(log n)
   */
  def balanced[A <% Ordered[A]](x: A, s: Int): BTree[A] = {
    def pair(ss: Int): (BTree[A], BTree[A]) =
      if (ss <= 0) (BTree.empty, BTree.empty)
      else {
